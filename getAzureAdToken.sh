...
# Get secret from GCP
AZURE_APP_SERVER_ID=$(kubectl exec -it $(kubectl get pods | grep sokos-nav-oppdrag | cut -f1 -d' ') -c sokos-nav-oppdrag -- env | grep -E "AZURE_APP_CLIENT_ID" | cut -d "=" -f 2 | tr -d '\r')
AZURE_OPENID_CONFIG_TOKEN_ENDPOINT=$(kubectl exec -it $(kubectl get pods | grep sokos-nav-oppdrag | cut -f1 -d' ') -c sokos-nav-oppdrag -- env | grep -E "AZURE_OPENID_CONFIG_TOKEN_ENDPOINT" | cut -d "=" -f 2 | tr -d '\r')

AZURE_APP_CLIENT_ID=$(kubectl exec -it $(kubectl get pods | grep sokos-nav-oppdrag | cut -f1 -d' ') -c sokos-nav-oppdrag -- env | grep -E "AZURE_APP_CLIENT_ID" | cut -d "=" -f 2 | tr -d '\r')
AZURE_APP_CLIENT_SECRET=$(kubectl exec -it $(kubectl get pods | grep sokos-nav-oppdrag | cut -f1 -d' ') -c sokos-nav-oppdrag -- env | grep -E "AZURE_APP_CLIENT_SECRET" | cut -d "=" -f 2 | tr -d '\r')

echo
echo "APP_NAME                              : sokos-nav-oppdrag"
